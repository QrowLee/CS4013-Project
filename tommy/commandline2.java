...
{    
   ...
   public void run(propertySystem2 machine)
         throws IOException
   {
      boolean more = true;
      while (more)
      { 
         ...
         switch (input) {
           ...
           String propertyToPay = in.nextLine();
           ArrayList<Payment2> temp = machine.getDuePayments(propertyToPay);
           for(int i = 0; i < temp.size(); i++){
            System.out.println(temp.get(i));
            }
           System.out.println("enter number of payments you want to make");
           int number = Integer.parseInt(in.nextLine());
           machine.pay(propertyToPay,name,number);
           break;
           case "add property":
